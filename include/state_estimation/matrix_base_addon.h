...
void UncorrelateCovariance(Index first) {
  static_assert(ColsAtCompileTime == RowsAtCompileTime,
...
void UncorrelateCovarianceSetVariance(Index first,
  ...
  static_assert(Width <= ColsAtCompileTime);
  assert(first + Width <= ColsAtCompileTime);
  derived().template block<Width, ColsAtCompileTime>(first, 0).setConstant(
      Scalar(0));
  derived().template block<ColsAtCompileTime, Width>(0, first).setConstant(
      Scalar(0));
